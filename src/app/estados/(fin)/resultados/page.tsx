...
export default async function ResultadosPage({ searchParams }: { searchParams: Promise<{ p?: string; vista?: string; meses?: string; anio?: string; mes?: string; contra?: string; ver?: string }> }) {
    ...
    <div className="space-y-3">
      ...
      {ver === "cifras" && (
        <StatementMatrix
          ...
          conAcum={false}
          secciones={[
            { titulo: "Ingresos", tono: "bg-pos", arbol: m.ingresos, totalLabel: "Total ingresos", totalVals: m.totalIng.vals },
            { titulo: "Gastos", tono: "bg-gold", arbol: m.gastos, totalLabel: "Total gastos", totalVals: m.totalGas.vals },
          ]}
          filasFinales={[{ nombre: "Utilidad antes de impuestos", vals: m.utilAntes.vals, tipo: "total" }]}
        />
      )}
...
function InteranualAnalisis({ modo, meses }: { modo: "vertical" | "horizontal"; meses: { etiqueta: string; anio: number; mes: number }[] }) {
  const a = analisisMatriz("er", modo, meses, "anio");
